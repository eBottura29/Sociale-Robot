...
Maak een standalone `.exe` van de desktop app (firmware GUI):

```bash
python -m pip install pyinstaller
pyinstaller --onefile --noconsole --name nier ^
  --add-data "src/settings/settings.json;settings" ^
  src/desktop_app/firmware/main.py
```

Optioneel, als je een HuggingFace token gebruikt:
```bash
pyinstaller --onefile --noconsole --name nier ^
  --add-data "src/settings/settings.json;settings" ^
  src/desktop_app/firmware/main.py
